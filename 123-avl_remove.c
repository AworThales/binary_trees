...
{
	avl_t *cur = node, *fresh_root = *root;
	int balance = 0;
	...
	while (cur != NULL)
	{
		...
		if (!((balance >= -1) && (balance <= 1)))
		{
			...
			{
				fresh_root = binary_tree_rotate_right(cur);
			}
			...
			{
				fresh_root = binary_tree_rotate_left(cur);
			}
			else if ((balance == 2) && (binary_tree_balance(cur->left) == -1))
			{
			...
			fresh_root = (cur == *root ? fresh_root : *root);
			break;
		}
		cur = cur->parent;
	}
...
{
	avl_t *node = NULL, *fresh_root = root, *fresh_node = NULL, *parent = NULL;

	if (fresh_root != NULL)
	{
		(void)node;
		(void)fresh_node;
		parent = remove_node(&fresh_root, value);
		adjust_balance_1(&fresh_root, parent);
	}
	return (fresh_root);
}
